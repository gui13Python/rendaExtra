...
// Busca a cotação em tempo real de QUALQUER ticker via brapi.dev (API v2).
// Sem token, só funciona para as 4 ações de teste da brapi: PETR4, VALE3,
// ITUB4 e MGLU3. Para qualquer outro ticker (TAEE3, TAEE4, TAEE11, etc.),
// é obrigatório configurar um token grátis em brapi.dev/dashboard.
const TICKERS_TESTE_BRAPI = ['PETR4', 'VALE3', 'ITUB4', 'MGLU3'];

async function buscarCotacaoOnline(ticker){
  const tickerUpper = ticker.toUpperCase();
  const token = localStorage.getItem('redividendos_brapi_token') || '';

  if(!token && !TICKERS_TESTE_BRAPI.includes(tickerUpper)){
    return { erro: 'sem_token' };
  }

  const url = `https://brapi.dev/api/v2/stocks/quote?symbols=${encodeURIComponent(tickerUpper)}`;
  try{
    const resp = await fetch(url, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    if(!resp.ok){
      return { erro: resp.status === 401 || resp.status === 402 ? 'sem_token' : 'nao_encontrado' };
    }
    const dados = await resp.json();
    const item = dados && dados.results && dados.results[0];
    const info = item && item.data;
    if(!info || typeof info.regularMarketPrice !== 'number'){
      return { erro: 'nao_encontrado' };
    }
    return { preco: info.regularMarketPrice, nome: info.longName || info.shortName || tickerUpper };
  }catch(e){
